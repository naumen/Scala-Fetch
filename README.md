...
**Fetch** - это библиотека для упрощения и оптимизации доступа к данным из таких источников как файловые системы, БД и веб-сервисы. Fetch основана на Cats и Cats Effect. Библиотека умеет предназначена для оптимизации выполнения запросов. Она позволяет:
...
Fetch может объединять запросы к одному источнику в один запрос. Чтобы показать библиотеке, что запросы можно выполнять независимо - их нужно связать аппликативным оператором. После этого их можно передавать в `Fetch.run` как раньше. Например:
...
Предположим, у нас есть дополнительный источник, который выдает случайные целочисленные до какой-то границы:
...
В обоих описанных случаях - при кобминировани и при объединении - всегда происходит дедупликация запросов. Это похоже на кэширование - один и тот же ID у одного источника запрашивается только единожды, но происходит только в пределах одного запроса. Кэширование же позволяет накапливать и передавать историю результатов между запросами.
...
Помимо `flatMap` в Fetch возможно использовать и другие комбинаторы. Например, `sequence` и `traverse` (последний уже был затронут выше) из Cats. Смысл этих комбинаторов - преобразовать тип `List[Fetch[_,_]]` в `Fetch[_, List[_]]`, который затем передаётся в `Fetch.run`. `sequence` предназначена для создания монады, содержащей коллекцию, из коллекции монад. `traverse` предназначена для создания монады, содержащей коллекцию, из коллекции каких-либо типов.
...
Этот запрос выполняется с ошибкой. Например, в ответе можно увидеть `List(8, 8, 8, 8, 8)`. Это связано с дедупликацией данных с одинаковым ID. Дело в том, идентификатор по логике Fetch всегда должен возвращать одинаковое значение. Поэтому реализация источника случайных чисел на Fetch - решение спорное. 
